...
function layerToWatermarkText(
  ...
  const style: NonNullable<TextOptions['style']> = {
    ...
    bold: true,
    textAlign: layerRtl ? 'right' : align,
    shadowStyle: {
      dx: 0,
      dy: 1,
      radius: shadowRadius,
  ...
  return {
    text: layer.text,
    position: {
      X: `${Math.round(layer.nx * 10000) / 100}%`,
      Y: `${Math.round(layer.ny * 10000) / 100}%`,
    },
    style,
  };
}
...
export async function markImageUriWithText(params: {
  ...
  const watermarkTexts = params.annotation.textLayers.map((layer) =>
    layerToWatermarkText(layer, params.imageWidth, params.language),
  );

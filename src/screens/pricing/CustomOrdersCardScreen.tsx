...
type CustomOrdersCardRoute = RouteProp<PricingStackParamList, 'CustomOrdersCard'>;

const CustomOrdersCardScreen: React.FC = () => {
  const {t} = useTranslation();
  const {theme} = useTheme();
  const {params} = useRoute<CustomOrdersCardRoute>();
  const {menus, header} = useOrdersHomeCardSectionAdmin({
    ordersHomeCardId: params.ordersHomeCardId,
    fallbackTitle: t('ordersHomeCustomCard'),
  });
    ...
    <ScreenContainer scroll={false} style={{padding: 0}} contentStyle={{flex: 1, padding: 0}}>
      ...
      <View style={styles.panel}>
        <MirrorPricingConfirmedOrdersPanel
          showTitle={false}
          homeCardId={params.ordersHomeCardId}
...
export default CustomOrdersCardScreen;
